...
class MagoMagiaModel extends BaseModel {
    ...
    public function create(MagoMagia $magoMagia) {
        $this->query("INSERT INTO mago_magias (mago_id, magia_id) VALUES (:mago_id, :magia_id)", [
    ...
    public function delete(int $mago_id, int $magia_id) {
        $this->query("DELETE FROM mago_magias WHERE mago_id = :mago_id AND magia_id = :magia_id", [
            ':mago_id' => $mago_id,
            ':magia_id' => $magia_id
        ]);
    }
    ...
    public function getMagiasdoMago(int $mago_id): array {
        $stmt = $this->query("SELECT magias.id, magias.nome, magias.nivel_minimo
        FROM magias
        INNER JOIN mago_magias
        ON magias.id = mago_magias.magia_id
        WHERE mago_magias.mago_id = :mago_id", [':mago_id' => $mago_id]);
    ...
    public function getRelacao(int $mago_id, int $magia_id): ?MagoMagia {
        $stmt = $this->query("SELECT * FROM mago_magias WHERE mago_id = :mago_id AND magia_id = :magia_id", [
            ':mago_id' => $mago_id,
            ':magia_id' => $magia_id
        ]);
